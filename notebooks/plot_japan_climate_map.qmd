---
title: "Plot Japan Climate Map"
---

[日本語版はこちら (Japanese version)](plot_japan_climate_map_ja.qmd)

This chapter draws Japan climate maps using [tmap](https://r-tmap.github.io/tmap/index.html) and [rnaturalearth](https://docs.ropensci.org/rnaturalearth/articles/rnaturalearth.html).

```{r}
library(tmap)
library(rnaturalearth)
```

## Load data

```{r}
data_dir <- Sys.getenv("PROJECT_DATA_DIR")
sf_climate <- readRDS(file.path(
  data_dir,
  "climate_mesh_data_joined/climate_mesh_data_with_aridity_index.rds"
))
```

## Plot PET

```{r}
# renv::install("ropensci/rnaturalearthhires")
variable <- "PET_year"
japan <- ne_states(country = "Japan", returnclass = "sf")
sf_plot <- sf_climate[!is.na(sf_climate[[variable]]), ]
tm <- tm_shape(japan, crs = 4612) +
  tm_polygons(col = "gray90") +
  tm_shape(sf_plot[, variable], crs = 4612) +
  tm_symbols(
    shape = 20,
    fill = variable,
    size = 0.1,
    fill_alpha = 0.9,
    fill.scale = tm_scale_continuous(values = "viridis"),
    fill.legend = tm_legend(
      title = variable,
      reverse = TRUE,
      position = c("right", "bottom"),
      frame = FALSE,
      bg.color = "transparent"
    )
  ) +
  tm_layout(frame = FALSE, bg.color = "transparent")
print(tm)

output_dir <- "output"
dir.create(output_dir, showWarnings = FALSE, recursive = TRUE)
tmap_save(
  tm,
  filename = file.path(output_dir, paste0("japan_climate_map_", variable, ".png")),
  dpi = 300
)
```

![PET map result](../output/japan_climate_map_PET_year.png)

## Plot Aridity Index

```{r}
variable <- "aridity_index"
japan <- ne_states(country = "Japan", returnclass = "sf")
sf_plot <- sf_climate[!is.na(sf_climate[[variable]]), ]
tm <- tm_shape(japan, crs = 4612) +
  tm_polygons(col = "gray90") +
  tm_shape(sf_plot[, variable], crs = 4612) +
  tm_symbols(
    shape = 20,
    fill = variable,
    size = 0.1,
    fill_alpha = 0.9,
    fill.scale = tm_scale_continuous(values = "viridis"),
    fill.legend = tm_legend(
      title = variable,
      reverse = TRUE,
      position = c("right", "bottom"),
      frame = FALSE,
      bg.color = "transparent"
    )
  ) +
  tm_layout(frame = FALSE, bg.color = "transparent")
print(tm)

tmap_save(
  tm,
  filename = file.path(output_dir, paste0("japan_climate_map_", variable, ".png")),
  dpi = 300
)
```

![Aridity Index map result](../output/japan_climate_map_aridity_index.png)
